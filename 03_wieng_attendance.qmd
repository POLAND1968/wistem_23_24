...
**Attendance Distribution:** On average, 14.27 students attended each WiENG meeting. The graphs and tables below show the attendance data obtained for each of the 26 meetings where attendance was taken. 
...
nov_15_data <- wieng_attendance_data %>%
...
::: {.callout-note icon=false collapse="true"}
## Table for Graduation Race for November 15th
```{r}
# Table
nov_15_data %>%
  select(date, grad_year) %>%
...
mar_16_data <- wieng_attendance_data %>%
  filter(date == "Mar 16th") %>%
  select(date, topic, num_meetings, grad_year, race, white_non_white) %>%
  mutate(
    num_meetings = as_factor(num_meetings), 
    grad_year = as_factor(grad_year), 
...
```{r}
#| label: mar-16-num-meetings

# Graph
ggplot(data = mar_16_data, mapping = aes(x = num_meetings)) + 
  geom_bar(color = color, fill = fill) + 
  geom_text(
    stat = "count", 
    mapping = aes(label = ..count..), 
...
::: {.callout-note icon=false collapse="true"}
## Table for Number of Meetings Attended for March 16th
```{r}
# Table
mar_16_data %>%
  select(date, num_meetings) %>%
...
```{r}
#| label: mar-16-grad-year

# Graph
ggplot(data = mar_16_data, mapping = aes(x = grad_year, fill = grad_year)) + 
  geom_bar() + 
  scale_fill_manual(values = unique_colors) + 
  geom_text(
    stat = "count", 
...
::: {.callout-note icon=false collapse="true"}
## Table for Graduation Race for March 16th
```{r}
# Table
mar_16_data %>%
  select(date, grad_year) %>%
...
# Graph
ggplot(data = mar_16_data, mapping = aes(x = race, fill = race)) + 
  geom_bar() + 
  scale_fill_manual(values = unique_colors) + 
  geom_text(
    stat = "count", 
...
::: {.callout-note icon=false collapse="true"}
## Table for Federal Race for March 16th
```{r}
# Table
mar_16_data %>%
  select(date, race) %>%
...
```{r}
#| label: mar-16-two-race

## Graph
ggplot(data = mar_16_data, mapping = aes(x = white_non_white, fill = white_non_white)) +
  geom_bar() + 
  scale_fill_manual(values = unique_colors) +
  geom_text(
    stat = "count", 
...
::: {.callout-note icon=false collapse="true"}
## Table for White & Non-White for March 16th
```{r}
# Table
mar_16_data %>%
  select(date, white_non_white) %>%
